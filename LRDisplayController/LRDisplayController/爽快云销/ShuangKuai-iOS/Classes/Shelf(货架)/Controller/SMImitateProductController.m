...
@interface SMImitateProductController ()<UITextFieldDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate,SMSynthesizeBtnViewClickDelegate>
...
@implementation SMImitateProductController
...
- (void)viewDidLoad {
    ...
    if (self.pushedByAction) {//如果是从我的微推广界面push 过来的，则一进来就显示到全部活动页面
        [self allActionBtnClick];
    }
    if (self.pushedByDiscount) {
...
//    dictClasses[NSFontAttributeName] = KDefaultFontSmall;
...
//    [trustBtn addTarget:self action:@selector(synthesizeBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    //综合排序 按钮
//    SMSynthesizeBtn *synthesizeBtn = [SMSynthesizeBtn synthesizeBtn];
//    self.synthesizeBtn = synthesizeBtn;
//    [sequenceView addSubview:synthesizeBtn];
//    
//    [synthesizeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
//        
//        //        make.centerX.equalTo(self.mas_centerX);
//        make.top.equalTo(sequenceView.mas_top).with.offset(8);
//        make.bottom.equalTo(sequenceView.mas_bottom).with.offset(-8);
//        make.left.equalTo(sequenceView.mas_left).with.offset(30);
//        make.width.equalTo(@85);
//    }];
//    [synthesizeBtn addTarget:self action:@selector(synthesizeBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [salesVolumeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(sequenceView.mas_left).with.offset(KScreenWidth/2-33);
    ...
    if (isIPhone5) {
        ...
        width = 25* KMatch6Height;
    }else{
        width = 25* KMatch6pHeight;
    }
    ...
    [trustView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(sequenceView.mas_right).with.offset(-width);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.productTableView) {
        ...
        Favorites * fav = [Favorites new];
        fav.id = self.favID;
        actionVc.favorites = fav;
        actionVc.isPushCounter = YES;
        actionVc.isup = YES;
        ...
        Favorites * fav = [Favorites new];
        fav.id = self.favID;
        discountVc.favorites = fav;
        discountVc.isPushCounter = YES;
        discountVc.isup = YES;
            ...
            if (array.count>0) {
                ...
                if (!self.productTableView.hidden) {
                    if (!self.productTableView.mj_footer.isRefreshing) {
                        [self.dataProducts removeAllObjects];
                        NSArray * array = [LocalStorehouse MR_findAll];
                        for (LocalStorehouse * model in array) {
...
- (void)leftItemDidClick:(UIButton *)btn{
    ...
    SMPersonCenterViewController *personCenterVc = [[SMPersonCenterViewController alloc] init];
...
- (UITableView *)discountCouponTableView{
    if (_discountCouponTableView == nil) {
        ...
        _discountCouponTableView.frame = CGRectMake(0, self.topViewHeight, KScreenWidth, KScreenHeight - self.topViewHeight - KStateBarHeight );
    }
...
- (UITableView *)actionTableView{
    if (_actionTableView == nil) {
        ...
        _actionTableView.frame = CGRectMake(0, self.topViewHeight, KScreenWidth, KScreenHeight - self.topViewHeight - KStateBarHeight );
...
- (UITableView *)sequenceTableView{
    ...
    if (_sequenceTableView == nil) {
        ...
        _sequenceTableView.frame = CGRectMake(0, self.topViewHeight + self.sortHeight,KScreenWidth / 3.0, height * 2);
...
-(UITableView *)commissionTableview
{
    ...
    if (!_commissionTableview) {
        ...
        _commissionTableview.frame = CGRectMake(x, self.topViewHeight + self.sortHeight,KScreenWidth / 3.0, height * 2);
    }
...
- (UICollectionView *)collectionView{
    if (_collectionView == nil) {
        ...
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, self.topViewHeight+ self.topViewHeight, KScreenWidth, KScreenHeight - self.topViewHeight - self.topViewHeight - KStateBarHeight ) collectionViewLayout:layout];
...
- (UITableView *)productTableView{
    if (_productTableView == nil) {
        ...
        _productTableView.frame = CGRectMake(0, self.topViewHeight+ self.sortHeight, KScreenWidth, KScreenHeight - self.topViewHeight - self.sortHeight - KStateBarHeight);
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (image) {
        [self.leftIconBtn setBackgroundImage:image forState:UIControlStateNormal];
    }
...
{
    //全部产品
    NSArray * array = [LocalStorehouse MR_findAllInContext:[NSManagedObjectContext MR_defaultContext]];
    for (NSInteger i = 0; i<array.count; i++) {
        if (i>10) {
            break;
        }
    ...
    //全部活动
    NSArray * actionArray = [LocalActivity MR_findAllInContext:[NSManagedObjectContext MR_defaultContext]];
