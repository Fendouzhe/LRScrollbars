...
- (void)setCollectionViewOffset{
    NSInteger rows = self.assetModels.count / COL;
    if (self.assetModels.count % COL > 0) {
        //最终显示出来的行数
        rows += 1;
    }
    CGFloat offsetH = (self.cellHeight + MARGIN) * rows + MARGIN;
    self.collectionView.contentOffset = CGPointMake(0, offsetH);
}
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    ...
    SMLog(@"self.assetModels.count  %zd",self.assetModels.count);
...
- (void)buttonClicked:(UIButton *)sender{
    if (self.sumSelected >= 9 && sender.selected == NO) {
        SMLog(@"超过了9张");
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"一次最多发送9张图片" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
        return;
    }
