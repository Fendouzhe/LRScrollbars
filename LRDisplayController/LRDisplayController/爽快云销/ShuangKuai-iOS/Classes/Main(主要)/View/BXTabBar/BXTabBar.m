...
- (void)setItems:(NSArray *)items
{
    ...
    for (int i = 0; i < items.count; i++) {
        ...
        [btn setTitleColor:KRedColorLight forState:UIControlStateSelected];
...
- (void)setSeletedIndex:(NSInteger)seletedIndex {
    ...
    UIButton *button = [self viewWithTag:BXTabBarTag + seletedIndex];//[self viewWithTag:(BXTabBarTag + seletedIndex)];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    CGFloat pointW = 43;
    CGFloat pointH = 61;
    CGFloat pointX = (KScreenWidth - pointW) / 2;
    CGFloat pointY = -12;
