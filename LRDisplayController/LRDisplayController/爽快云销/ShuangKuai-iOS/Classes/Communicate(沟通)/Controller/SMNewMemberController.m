...
@interface SMNewMemberController ()<SMContactPersonTableViewCellDelegate,ChatTopOfTableViewDelegate,SMAddGroupTeamViewControllerDelegate>
...
@implementation SMNewMemberController
...
- (void)viewDidLoad {
    ...
    [topView setWithImageArray:@[@""] withStrArray:@[@"我的团队"]];
    self.tableView.tableHeaderView = topView;
    
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(addNewMenmberToGroup)];
}

//确定点击
-(void)addNewMenmberToGroup{
    
    NSString *groupName = self.roomDetail.chatroom.roomName;
    ...
    [[SKAPI shared] joinGroup4IM:groupId andGroupName:groupName andUserIds:useridArray block:^(id result, NSError *error) {
        SMLog(@"result = %@------- error = %@",result,error);
        [[NSNotificationCenter defaultCenter] postNotificationName:AddGroupMemberNotification object:nil userInfo:nil];
        ...
        if (!error) {
            ...
            for (User *user in array) {
                ...
                [self.chatroomMemberList enumerateObjectsUsingBlock:^(ChatroomMemberListData *listData, NSUInteger indx, BOOL * _Nonnull stop) {
                    ...
                    } else if (![user.userid isEqualToString:listData.userId] && indx == self.chatroomMemberList.count-1) {
...
//            SMLog(@"self.chatroomMemberList = %@",self.chatroomMemberList);
//            SMLog(@"self.dataSource = %@----%lu",self.dataSource,self.dataSource.count);
