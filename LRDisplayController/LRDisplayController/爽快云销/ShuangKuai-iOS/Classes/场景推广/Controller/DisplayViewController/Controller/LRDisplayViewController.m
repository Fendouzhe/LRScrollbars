...
- (void)setSelectIndex:(NSInteger)selectIndex{
    ...
    if (self.titleLabels.count) {
        LRDisplayTitleLabel *label = self.titleLabels[selectIndex];
...
//        [self.titleWidths addObject:@(width)];
        //计算标题大小
        CGFloat width = LRTitleScrollViewTitleWidth;//LRScreenW/count;
        //添加标题宽度进数组
        [self.titleWidths addObject:@(width)];
        totalWidth += width;
    }
...
- (void)setUpAllTitle{
    ...
    for (int i = 0; i < count; i++) {
        ...
        //LRDisplayTitleLabel *label = [[LRDisplayTitleLabel alloc] init];
        LRDisplayTitleLabel *label = [LRDisplayTitleLabel displayTitleLabel];
        label.tag = i;
        //label.textColor = self.normalColor;
        //label.textColor = [UIColor whiteColor];
        label.backgroundColor = KBlackColorLight;
        //label.font = self.titleFont;
        //label.text = vc.title;
        label.title = vc.title;
        label.image = [UIImage imageNamed:[NSString stringWithFormat:@"tuiguang%d",i]];
        //获取标题宽度
        labelW = [self.titleWidths[i] floatValue];
        //获取上一个标签
        LRDisplayTitleLabel *lastLabel = [self.titleLabels lastObject];
        //计算x坐标
...
//        leftLabel.textColor = leftColor;
        //右边颜色
        rightLabel.backgroundColor = rightColor;
        //左边颜色
        leftLabel.backgroundColor = leftColor;
...
/// 标题缩放 ---- 网易新闻 ---------
- (void)setupTitleScaleWithOffset:(CGFloat)offsetX rightLabel:(LRDisplayTitleLabel *)rightLabel leftLabel:(LRDisplayTitleLabel *)leftLabel{
...
/// 获取两个标题按钮宽度差值
- (CGFloat)widthDeltaWithRightLabel:(LRDisplayTitleLabel *)rightLabel leftLabel:(LRDisplayTitleLabel *)leftLabel{
...
//    CGRect titleBoundsL = [leftLabel.text boundingRectWithSize:CGSizeMake(MAXFLOAT, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleFont} context:nil];
    CGRect titleBoundsR = [rightLabel.title boundingRectWithSize:CGSizeMake(MAXFLOAT, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleFont} context:nil];
    
    CGRect titleBoundsL = [leftLabel.title boundingRectWithSize:CGSizeMake(MAXFLOAT, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleFont} context:nil];
    
    return titleBoundsR.size.width - titleBoundsL.size.width;
}
/// 设置下划线偏移 ---- 喜马拉雅 -----
- (void)setupUnderLineOffset:(CGFloat)offsetX rightLabel:(LRDisplayTitleLabel *)rightLabel leftLabel:(LRDisplayTitleLabel *)leftLabel{
    //如果点击了标题就不再进行后面的计算
...
/// 设置遮盖偏移 ------ 腾讯使用-------
- (void)setUpCoverOffset:(CGFloat)offsetX rightLabel:(LRDisplayTitleLabel *)rightLabel leftLabel:(LRDisplayTitleLabel *)leftLabel{
    //如果点击了标题就不再进行后面的计算
    if (_isClickTitle) return;
    // 获取两个标题中心点距离
    CGFloat centerDelta = rightLabel.x - leftLabel.x;
...
- (void)titleLabelClick:(UITapGestureRecognizer *)tap{
    ...
    _isClickTitle = YES;
    //获得点击的标题label
    LRDisplayTitleLabel *label = (LRDisplayTitleLabel *)tap.view;
...
/// 选中标签
- (void)selectLabel:(LRDisplayTitleLabel *)label{
    //遍历数组标签,未选中的状态还原
    for (LRDisplayTitleLabel *titleLabel in self.titleLabels) {
        ...
        //新需求
        titleLabel.backgroundColor = self.normalColor;
    ...
    if ([label.title isEqualToString:@"拼团促销"]) {
        //移除已读的拼团推送消息
    ...
    }else if ([label.title isEqualToString:@"活动邀请"]){
        //移除已读的活动推送消息
    ...
    }else if ([label.title isEqualToString:@"秒杀抢购"]){
        //移除已读的秒杀推送消息
...
/// 设置标题居中
- (void)setLabelTitleCenter:(LRDisplayTitleLabel *)label{
...
/// 设置下标的位置
- (void)setUpUnderLine:(LRDisplayTitleLabel *)label{
    //获得文字尺寸
//    CGSize titleSize = [label.text boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleFont} context:nil].size;
    CGSize titleSize = [label.title boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleFont} context:nil].size;
    //计算下划线
...
/// 设置cover
- (void)setUpCoverView:(LRDisplayTitleLabel *)label{
    //获得文字尺寸
//    CGSize titleSize = [label.text boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleFont} context:nil].size;
    CGSize titleSize = [label.title boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.titleFont} context:nil].size;
    CGFloat border = 5;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //vc.view.frame = CGRectMake(0, 0, self.contentCollectionView.width, LRScreenH-LRNavBarH-LRTitleScrollViewH-49);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    //***********************新需求***********************//
    //超过屏幕一半就选中
    int index = offsetX/LRScreenW + 0.5;
    //选中对应标题
