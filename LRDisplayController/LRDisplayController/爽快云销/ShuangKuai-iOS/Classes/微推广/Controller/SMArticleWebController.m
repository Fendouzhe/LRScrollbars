...
@interface SMArticleWebController ()<SMShareMenuDelegate,UIWebViewDelegate,UIScrollViewDelegate,SMShareToWXMenuDelegate,SMShareMenuDelegate>
...
@implementation SMArticleWebController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.title = self.listModle.title;
...
- (void)setupWebview{
    ...
    NSString *baseUrl = [NSString stringWithFormat:@"%@spread_news_detail.html",SKAPI_PREFIX_SHARE];
    //NSString *urlStr = [baseUrl stringByAppendingString:[NSString stringWithFormat:@"?namid=%@&imagePath=%@",self.listModle.id,self.listModle.imagePaths]];
    NSString *urlStr = [baseUrl stringByAppendingString:[NSString stringWithFormat:@"?namid=%@",self.listModle.id]];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    ...
    if ([requestString rangeOfString:@"imagePath"].location == NSNotFound) {
        ...
        NSString *str = request.URL.absoluteString;
        str = [str stringByAppendingString:[NSString stringWithFormat:@"&imagePath=%@",_listModle.imagePaths]];
        //self.urlstr = str;
        [webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:str]]];
        ...
        self.image = [image scaleToSize:CGSizeMake(100, 100)];
