...
@interface SMForgetPayViewCOntroller ()<UITextFieldDelegate>
...
@implementation SMForgetPayViewCOntroller
...
- (void)nextStepClick{
    ...
    if (self.phoneField.text.length != 11 || self.testCodeField.text.length != 6) {
        
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"输入有误，请重新输入" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:nil, nil];
        [alert show];
    }else{
        [self getAPI];
    }
...
- (void)getAPI{
    [[SKAPI shared] smsVerifyWithPhone:self.phoneField.text andVefifyCode:self.testCodeField.text block:^(id result, NSError *error) {
        if (error) {
            SMLog(@"%@",error);
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"验证码输入错误" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:nil, nil];
            [alert show];
        }else{
            if ([result[@"message"] isEqualToString:@"success"]) {
                SMNewPasswordController *vc = [[SMNewPasswordController alloc] init];
