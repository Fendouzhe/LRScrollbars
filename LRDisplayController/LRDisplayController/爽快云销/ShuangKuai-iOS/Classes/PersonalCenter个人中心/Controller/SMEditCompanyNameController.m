...
@interface SMEditCompanyNameController ()

@property (weak, nonatomic) IBOutlet UITextField *inputField;

@end

@implementation SMEditCompanyNameController
...
- (void)rightBtnClick{
    ...
    NSDictionary *dict = @{@"companyName": self.inputField.text};
    [[SKAPI shared] editProfile:dict block:^(id result, NSError *error) {
        if (!error) {
            SMLog(@"更改性别 成功");
            SMLog(@"%@",result);
            //存到本地
            [[NSUserDefaults standardUserDefaults] setObject:self.inputField.text forKey:KUserCompanyName];
