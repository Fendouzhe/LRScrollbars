...
@interface SMBoundCardViewController ()<UITextFieldDelegate,UIPickerViewDelegate,UIPickerViewDataSource>
/**
 *  持卡人
 */
@property (weak, nonatomic) IBOutlet UITextField *ownnerField;
/**
 *  卡号
 */
...
@property (weak, nonatomic) IBOutlet UITextField *subBankField;
 ...
 *  展开下面pickerView 的按钮
 */
@property (weak, nonatomic) IBOutlet UIButton *openedBtn;
/**
 *  发卡的银行选择器数组
 */
...
@property (weak, nonatomic) IBOutlet UIView *grayLine3;

@property (weak, nonatomic) IBOutlet UIView *grayLine4;


@end

@implementation SMBoundCardViewController
...
- (void)setupColor{
    self.title = @"绑定银行卡";
    self.nextStepBtn.backgroundColor = KRedColorLight;
    ...
    self.grayLine2.backgroundColor = KGrayColorSeparatorLine;
    self.grayLine3.backgroundColor = KGrayColorSeparatorLine;
    self.grayLine4.backgroundColor = KGrayColorSeparatorLine;
...
- (IBAction)nextStepBtnClick {
    SMLog(@"点击了  下一步 按钮");
    //做出判断  输入为空 提醒输入
    if (![self.ownnerField.text isEqualToString:@""]) {
        if (![self.cardNumField.text isEqualToString:@""]) {
            if (![self.bankField.text isEqualToString:@""]) {
                //判断银行卡是否正确
                if ([self checkCardNo:self.cardNumField.text] ) {
                    //判断身份证号码是否为空以及是否正确
                    if ([self.idCardField.text isEqualToString:@""]) {
                        [self showAlertViewWithMessage:@"请输入身份证号码"];
                        [self hideAlertView];
                    }else
                    {
                        if ([self checkId:self.idCardField.text]) {
                            SMMobileVerificationViewController *vc = [[SMMobileVerificationViewController alloc] init];
                            //将银行卡号传过去。。
                            vc.cardNum = self.cardNumField.text;
                            vc.cardName = self.ownnerField.text ;
                            vc.idCareNum = self.idCardField.text;
                            vc.subBank = self.subBankField.text;
                            [self.navigationController pushViewController:vc animated:YES];
//                            [self boundCard];
                        }else
                        {
                            [self showAlertViewWithMessage:@"身份证号输入有误，请检查后重新输入"];
                            [self hideAlertView];
                        }
                    }
                }else
                {
                    [self showAlertViewWithMessage:@"银行卡号输入有误，请检查后重新输入"];
                    [self hideAlertView];
                }
            }else
            {
                [self showAlertViewWithMessage:@"请选择开卡银行"];
                [self hideAlertView];
            }
            
        }else
        {
            [self showAlertViewWithMessage:@"请输入卡号"];
            [self hideAlertView];
        }
    }else
    {
        [self showAlertViewWithMessage:@"请输入持卡人姓名"];
        [self hideAlertView];
    }
...
- (void)boundCard{
    SMLog(@"self.subBankField.text   %@",self.subBankField.text);
    if (self.subBankField.text == nil) {
        self.subBankField.text = @"";
    }
    
    [[SKAPI shared] bindCard:self.cardNumField.text andIdCard:self.idCardField.text andName:self.ownnerField.text andSubBank:self.subBankField.text type:1 block:^(id result, NSError *error) {
        if (!error) {
            SMLog(@"result bindCard %@",result);
        }else{
            SMLog(@"error bindCard %@",error);
        }
    }];
}
...
//自动判断 不需要了
- (IBAction)openBanksBtnClick:(UIButton *)sender {
//    SMLog(@"点击了  展开银行列表 按钮");
//    //这个需要真实网络数据 再做好一点］
//    self.openedBtn.selected = !self.openedBtn.selected;
//    self.pickerView.hidden = !self.openedBtn.selected;
//    self.toolBar.hidden = !self.openedBtn.selected;
//    self.completeBtn.hidden = !self.openedBtn.selected;
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (textField == self.ownnerField) {
        if ([string isEqualToString:@"\n"]) {
            [self.cardNumField becomeFirstResponder];
        }
    }else if(textField == self.cardNumField){
        if ([string isEqualToString:@"\n"]) {
            //判断开卡银行
            [self.bankField becomeFirstResponder];
        }
    }else if (textField == self.bankField){
        if ([string isEqualToString:@"\n"]) {
            [self.bankField resignFirstResponder];
        }
    }else if(textField == self.idCardField){
        if ([string isEqualToString:@"\n"]) {
            [self.idCardField resignFirstResponder];
        }
    }else if (textField == self.subBankField){
        if ([string isEqualToString:@"\n"]) {
            [self.idCardField resignFirstResponder];
        }
    }
...
//        self.bankField.inputAccessoryView = self.toolBar;
//        self.bankField.inputView = self.pickerView;
//    }
//    return YES;
//}
...
- (UIView *)toolBar{
    if (_toolBar == nil) {
        ...
        CGFloat toolBarH = 44;
        CGFloat toolBarY = CGRectGetMinY(self.pickerView.frame) - toolBarH;
        _toolBar.frame = CGRectMake(0, toolBarY, KScreenWidth, toolBarH);
        ...
        NSAttributedString *str = [[NSAttributedString alloc] initWithString:@"完成" attributes:dict];
        [completeBtn setAttributedTitle:str forState:UIControlStateNormal];
        self.completeBtn = completeBtn;
        [_toolBar addSubview:completeBtn];
        [self.completeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            
            make.centerY.equalTo(_toolBar.mas_centerY);

            make.right.equalTo(_toolBar.mas_right).with.offset(-10);
...
- (void)completeBtnClick{
    SMLog(@"点击了 完成按钮");
    //这里可以通过选中的是哪一行   然后去模型数组中拿到对应位置的模型数据  赋值给bankField 里的text
    NSInteger row = [self.pickerView selectedRowInComponent:0];
    SMLog(@"%zd",row);
    if (row>0) {
        self.bankField.text = self.array[row-1];
    }
    
    self.pickerView.hidden = YES;
    self.completeBtn.hidden = YES;
    self.openedBtn.selected = NO;
...
{
    if (!_array) {
        _array = @[@"中国银行",@"中国建设银行",@"招商银行",@"中国农业银行",@"中国工商银行",@"广州民生银行"];
    }
    return _array;
}
...
-(void)loadBnakWithBankCard:(NSString * )string
{
    int a[] = { 621098, 622150, 622151, 622181, 622188, 955100, 621095, 620062, 621285,
    ...
    NSArray *arr2 = @[@"邮储银行·绿卡通@",@"邮储银行·绿卡银联标准卡@", @"邮储银行·绿卡银联标准卡@", @"邮储银行·绿卡专用卡@", @"邮储银行·绿卡银联标准卡@",
    ...
    //截取卡号的前6位
    if (string.length>=6) {
       NSString * str = [string substringWithRange:NSMakeRange(0, 6)];
        for (int i=0; i< sizeof(a)/sizeof(int); i++) {
            if (str.intValue == a[i]) {
                self.bankField.text = [NSString stringWithFormat:@"%@",[arr2[i] substringWithRange:NSMakeRange(0, [arr2[i] length]-2)]];
                return;
            }
            if (i==sizeof(a)/sizeof(int)-1) {
                self.bankField.text = @"  银行卡输入错误，请重新输入";
            }
        }
    }else
    {
...
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(searchFieldchange) name:UITextFieldTextDidChangeNotification object:self.cardNumField];
}
-(void)searchFieldchange
{
    [self loadBnakWithBankCard:self.cardNumField.text];
}
...
-(BOOL)checkId:(NSString *)ID
{
    if (ID.length == 15 || ID.length == 18) {
        ...
        bool sfzNo = [emailTest evaluateWithObject:[ID stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
        ...
        if (ID.length == 15) {
            ...
            bool sfz18NO = [self checkIdentityCardNo:ID];
            if (!sfz18NO) {
